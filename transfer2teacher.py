...
def parse_option():
    ...
    parser.add_argument('--tb_freq', type=int,
                        default=10, help='tb frequency')
    parser.add_argument('--save_freq', type=int,
                        default=100, help='save frequency')
    parser.add_argument('--batch_size', type=int,
                        default=45, help='batch_size')
    ...
    parser.add_argument('--lr_decay_epochs', type=str,
                        default='20,40', help='where to decay lr, can be a list')
    ...
    parser.add_argument('--path_t', type=str, default="./save/models/Teacher_resnet50_epochs30_alpha-5.0_T50_a=-5/resnet50_best.pth",
                        help='teacher model checkpoint')
    parser.add_argument('--d_rep', type=int, default=128,
                        help="dimension of representation layer")
    parser.add_argument('--dataset', type=str, default='boe',
                        choices=['oct2', 'boe'], help='dataset')
    parser.add_argument('-T', '--temperature', type=float,
                        default=50, help='temperature')
    parser.add_argument('-a', '--alpha', type=float,
                        default=-5, help='alpha multiplier')
    ...
    parser.add_argument('-m', '--method', type=str, choices=['ALL', 'FE', 'HL'],
    ...
    if method == 'ALL':
        opt.model_name = 'ALL_S_{}_T_{}_{}_{}'.format(
            opt.model_s, opt.model_t, opt.dataset,  opt.info)
    elif method == 'HL':
        opt.model_name = 'HL_S_{}_T_{}_{}_{}'.format(
            opt.model_s, opt.model_t, opt.dataset,  opt.info)
    elif method == 'FE':
        opt.model_name = 'FE_S_{}_T_{}_{}_{}'.format(
            opt.model_s, opt.model_t, opt.dataset,  opt.info)
...
def main():
    ...
    if method == 'ALL':
        model = load_model(model_dict=model_dict,
                           model_path=opt.path_t, d_rep=opt.d_rep, n_cls=4)
