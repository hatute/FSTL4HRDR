...
def student_parse_option():
    ...
    parser.add_argument('--save_freq', type=int,
                        default=500, help='save frequency')
    ...
    parser.add_argument('-a', '--alpha', type=float,
                        default=0.5, help='alpha multiplier')
    parser.add_argument('-b', '--beta', type=float,
                        default=0.5, help='weight for classification')
    ...
    parser.add_argument('-K', '--k_fold', type=int,
                        default=5, help='number of k fold')
    ...
    opt.model_name = f'STKD{opt.trial}_S_{opt.model_s}_T_{opt.model_t}_{opt.train_dataset}{opt.test_dataset}_a{opt.alpha}_b{opt.beta}_KDT{opt.kd_T}'
...
def main():
    ...
    if opt.train_dataset == opt.test_dataset:
        print("k-fold mode")
        k_fold(opt=opt, module_list=module_list, criterion_list=criterion_list, optimizer=optimizer, logger=logger)
    else:
        print("hybird mode")
        hybird(opt=opt, module_list=module_list, criterion_list=criterion_list, optimizer=optimizer, logger=logger)
...
def k_fold(opt, module_list, criterion_list, optimizer, logger):
    ...
    for idx, (train_loader, val_loader) in enumerate(oct2.get_kfold_dataloader(k=k, c_dataset=opt.train_dataset,
                                                                     batch_size=opt.batch_size,
                                                                     num_workers=opt.num_workers)):

        val_teacher(val_loader=val_loader, model_t=module_list[-1], criterion_cls=criterion_cls, opt=opt)
        ...
        save_file = os.path.join(
            opt.save_folder, f"F{idx + 1}{opt.model_s}_last.pth")
        torch.save(state, save_file)
...
def hybird(opt, module_list, criterion_list, optimizer, logger):
    ...
    train_loader, val_loader = oct2.get_hybird_dataloaders(c_train=opt.train_dataset, c_test=opt.test_dataset,
                                                           batch_size=opt.batch_size,
                                                           num_workers=opt.num_workers)
    val_teacher(val_loader=val_loader, model_t=module_list[-1], criterion_cls=criterion_cls, opt=opt)
    # add timer
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        logger.log_value(f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_train_acc', train_acc, epoch)
        logger.log_value(f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_train_loss', train_loss, epoch)

        test_acc, test_loss = validate(
            val_loader, module_list[0], criterion_cls, opt)

        logger.log_value(f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_test_acc', test_acc, epoch)
        logger.log_value(f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_test_loss', test_loss, epoch)
        ...
        if test_acc > best_acc:
            ...
            save_file = os.path.join(
                opt.save_folder, f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_{opt.model_s}_best.pth')
    ...
    # This best accuracy is only for printing purpose.
    # The results reported in the paper/README is from the last epoch.
    print(f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_{opt.model_s}_best accuracy:', best_acc)
    ...
    save_file = os.path.join(
        opt.save_folder, f'TrainDS_{opt.train_dataset}_TestDS_{opt.test_dataset}_{opt.model_s}_last.pth')
    torch.save(state, save_file)
    end_time = time.time()
    print(time.strftime("%Hh:%Mm:%Ss", time.gmtime(end_time - start_time)))
